...
class TestEnrichDataframe(unittest.TestCase):
    ...
    def xtest_get_df_comuni(self):
        df = get_df_comuni()
        self.assertTrue(isinstance(df, pd.DataFrame))
        self.assertGreater(df.shape[0], 0)

    def xtest_get_df_province(self):
        df = get_df_province()
        self.assertTrue(isinstance(df, pd.DataFrame))
        self.assertGreater(df.shape[0], 0)

    def xtest_get_df_regioni(self):
        df = get_df_regioni()
        self.assertTrue(isinstance(df, pd.DataFrame))
        self.assertGreater(df.shape[0], 0)

    def xtest_get_list_comuni(self):
        result = get_list_comuni()
        self.assertTrue(isinstance(result, list))
        self.assertGreater(len(result), 0)

    def xtest_get_list_province(self):
        result = get_list_province()
        self.assertTrue(isinstance(result, list))
        self.assertGreater(len(result), 0)

    def xtest_get_list_regioni(self):
